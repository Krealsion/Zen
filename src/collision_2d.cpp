...
Rectangle Collision2D::get_bounding_box(const std::vector<Vector2>& points) {
  Rectangle rect;
  if (points.empty()) {
    return rect;
  }
  ...
  rect.set_x(min_x);
  rect.set_y(min_y);
  rect.set_width(max_x - min_x);
  rect.set_height(max_y - min_y);
  return rect;
}
...
bool Collision2D::bounding_line_collision_check(Line2D l1, Line2D l2) const {
  ...
  if (bounding_box_collision_check(get_bounding_box(set_1), get_bounding_box(set_2))) {
    for (const auto& i : line_set_1) {
      for (const auto& j : line_set_2) {
        auto p = get_line_collision(i, j); // Returns any points of collision (0, 1 or 2)
